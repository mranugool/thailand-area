 ...
 * @since  1.6
 */
class ThailandareaModelDistricts extends \Joomla\CMS\MVC\Model\ListModel
{
...
/**
	...
	*/
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'districts_code', 'a.`districts_code`',
				'districts_name_th', 'a.`districts_name_th`',
				'districts_name_en', 'a.`districts_name_en`',
				'districts_geo_id', 'a.`districts_geo_id`',
				'districts_postcode', 'a.`districts_postcode`',
	...
	protected function getListQuery()
	{
		...
		$query->from('`#__thailand_districts` AS a');
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('( a.districts_code LIKE ' . $search . '  OR  a.districts_name_th LIKE ' . $search . '  OR  a.districts_name_en LIKE ' . $search . '  OR  a.districts_postcode LIKE ' . $search . ' )');
			}
		}
